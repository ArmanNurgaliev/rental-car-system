...
public class GatewayConfig {
    ...
    public RouteLocator routes(RouteLocatorBuilder builder) {
        return builder.routes()
                ...
                .route("reservation-service", r -> r.path("/api/reservation/**").filters(f -> f.filter(filter)).uri("lb://reservation-service"))
                .route("vehicle-service", r -> r.path("/api/vehicle/**").filters(f -> f.filter(filter)).uri("lb://vehicle-service"))
