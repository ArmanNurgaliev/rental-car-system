...
public interface ReservationRepository extends JpaRepository<Reservation, Long> {
    ...
    @Query("SELECT r FROM Reservation r WHERE r.vehicleId = ?1 AND r.returnDate < ?2 AND r.returnLocationId != ?3 ORDER BY r.returnDate DESC LIMIT 1")
    ...
    Optional<Reservation> findReservationByVehicleAndDates(Long vehicleId, Date dueDate, Date returnDate);
}
